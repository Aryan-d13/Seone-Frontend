...
import type { TemplateJSON, ZoneSpec, BoundsSpec, StyleDef, AssetDef } from '../types/template';
...
function deriveEditableTextBounds(zone: ZoneSpec, resolvedZone: ResolvedZone | undefined): BoundsSpec | null {
    ...
    const lineSpacing = Math.max(0, coerceFiniteNumber(layout.line_spacing_px, zone.text.line_spacing_px ?? 0));
...
function boundsFromResolvedRect(resolvedZone: ResolvedZone | undefined): BoundsSpec | null {
    const rect = resolvedZone?.rect;
    if (!rect) return null;
...
function clampBoundsWithinContainer(bounds: BoundsSpec, container: BoundsSpec): BoundsSpec {
    ...
    const height = Math.min(
        Math.max(10, bounds.height !== undefined ? Number(bounds.height) || 10 : containerHeight),
        containerHeight,
    );

    return {
        x: clamp(Number(bounds.x) || containerX, containerX, containerX + containerWidth - width),
        y: clamp(Number(bounds.y) || containerY, containerY, containerY + containerHeight - height),
        width,
        height,
    };
}
...
function splitTextBackgroundLayers(
    ...
    for (const zone of template.zones) {
        ...
        const existingBackgroundStyle =
            existingBackgroundZone?.style_ref ? nextStyles[existingBackgroundZone.style_ref] : undefined;
        ...
        const backgroundFill =
            (typeof existingBackgroundStyle?.fill === 'string' && existingBackgroundStyle.fill) ||
            (typeof existingBackgroundStyle?.bg_fill === 'string' && existingBackgroundStyle.bg_fill) ||
        ...
        const migratedZone: ZoneSpec = normalizeZone({
            ...zone,
            bounds: clampBoundsWithinContainer(editableBounds ?? zone.bounds, canonicalBackgroundBounds),
            text: zone.text
                ? {
                    ...zone.text,
                    width_percent: 100,
...
export const useTemplateStore = create<TemplateStore>((set, get) => {
    ...
    return {
        ...
        setCanvasSize: (w, h) => {
            pushHistory();
            set((s) => ({
                template: { ...s.template, canvas: { ...s.template.canvas, width: w, height: h } },
            }));
        },
        setTemplateId: (id) => {
            pushHistory();
        ...
        duplicateZone: (id) => {
            ...
            const dup: ZoneSpec = {
                ...
                bounds: {
                    ...
                    ...(!hasForcedAutoHeight(zone) && zone.bounds.height !== undefined ? { height: zone.bounds.height } : {}),
                },
            };
            set((s) => ({
        ...
        loadFromManifest: (manifest) => {
            ...
            for (const zone of normalizedTemplate.zones) {
                if (zone.type !== 'text' || !zone.content_ref || previewTexts[zone.content_ref]) continue;
        ...
        setZoneClipTiming: (zoneId, start, end) => {
            set((s) => {
                ...
                const rectHeight =
                    typeof zone.bounds.height === 'number' ? zone.bounds.height : Number(zone.bounds.width) || 0;
                const nextResolvedZone = {
                    ...
                    resolved:
                        s.activeManifest.resolved_zones.find((entry) => entry.id === zoneId)?.resolved ?? {},
                ...
                return {
                    ...
                    template: {
                        ...
                        zones: s.template.zones.map((entry) =>
                            entry.id === zoneId ? normalizeZone({ ...entry, bounds: normalizedBounds }) : entry,
                        ),
                    },
                };
            });
