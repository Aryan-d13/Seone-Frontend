...
export default function CanvasWorkspace() {
  const { template, selectZone, gridSnap, gridSize, zoom, setZoom, interactionMode } =
    useTemplateStore();
  ...
  useLayoutEffect(() => {
    ...
    const centerStage = () => {
      viewport.scrollLeft = Math.max(
        0,
        workspacePadding + scaledW / 2 - viewport.clientWidth / 2
      );
      viewport.scrollTop = Math.max(
        0,
        workspacePadding + scaledH / 2 - viewport.clientHeight / 2
      );
    };
    ...
    const needsInitialPlacement =
      ...
      (workspacePadding > viewport.clientWidth / 2 ||
        workspacePadding > viewport.clientHeight / 2);
  ...
  useEffect(() => {
    ...
    const handleViewportWheel = (event: WheelEvent) => {
      ...
      const nextZoom = Math.min(
        Math.max(Number((zoom + direction * 0.1).toFixed(2)), 0.25),
        3
      );
