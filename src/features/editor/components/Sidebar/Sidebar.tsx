...
import {
    Type, Image, Film, Upload, Download, Undo2, Redo2, FlaskConical,
    Cloud, CloudUpload, Trash2, FolderOpen, FilePlus, LogOut, Loader2,
    FileDown,
} from 'lucide-react';
...
import {
    listTemplates, getTemplate, saveTemplate, deleteTemplate, toDocId,
...
export default function Sidebar({
    ...
    const {
        template, addZone, selectZone, selectedZoneId,
        undo, redo, canUndo, canRedo, setTemplate, activeManifest, previewTexts, setPreviewText, setZoneClipTiming,
    } = useTemplateStore();
    ...
    const overlayCount =
        zones.filter((zone) => zone.type !== 'video').length;
    const sourceLayerId = template.zones.find((zone) => zone.type === 'video')?.id ?? null;
    const sourceVideoName = sourceVideoKey ? sourceVideoKey.split('/').at(-1) || sourceVideoKey : 'Attached source';
        ...
        return (
            <aside className="sidebar sidebar--clip">
                ...
                <div className="sidebar__clip-actions">
                    <button className="sidebar__element-btn sidebar__element-btn--clip" onClick={handleAddText}>
                        <span className="sidebar__element-icon sidebar__element-icon--text"><Type size={15} /></span>
                        Text overlay
                    </button>
                    <button className="sidebar__element-btn sidebar__element-btn--clip" onClick={handleAddImage}>
                        <span className="sidebar__element-icon sidebar__element-icon--image"><Image size={15} /></span>
                        Graphic layer
                    </button>
                </div>
                ...
                <div className="sidebar__clip-bin">
                    {orderedLayerZones.map((zone) => {
                        ...
                        return (
                            ...
                            >
                                <span className={`sidebar__asset-thumb sidebar__asset-thumb--${zone.type}`}>
                ...
                <div className="sidebar__clip-footer">
                    ...
                    <div className="sidebar__actions">
                        <button className="sidebar__action-btn sidebar__action-btn--primary" onClick={handleExport}>
    ...
    return (
        <aside className="sidebar">
            <div className="sidebar__header">
                <div className="sidebar__brand">
                    ...
                    <div className="sidebar__brand-copy">
                        ...
                        <small>{isClipMode ? 'Clip layers and timing' : 'Template layouts and overlays'}</small>
                    </div>
            ...
            <div className="sidebar__section">
                <div className="sidebar__section-title">{isClipMode ? 'Add Overlays' : 'Elements'}</div>
                <div className="sidebar__elements">
                    <button className="sidebar__element-btn" onClick={handleAddText}>
                        <span className="sidebar__element-icon sidebar__element-icon--text"><Type size={16} /></span>
                        {isClipMode ? 'Text Overlay' : 'Text Zone'}
                    </button>
                    <button className="sidebar__element-btn" onClick={handleAddImage}>
                        <span className="sidebar__element-icon sidebar__element-icon--image"><Image size={16} /></span>
                        {isClipMode ? 'Image Layer' : 'Image Zone'}
                    </button>
                    {!isClipMode && (
                        <button className="sidebar__element-btn" onClick={handleAddVideo}>
                            <span className="sidebar__element-icon sidebar__element-icon--video"><Film size={16} /></span>
                            Video Zone
                        </button>
                    )}
                </div>
                ...
                <div className="sidebar__section">
                    ...
                    <div className="sidebar__summary">
                        <div className="sidebar__summary-item">
                            ...
                            <strong className="sidebar__summary-value" title={sourceVideoKey || 'Attached source video'}>
                                {sourceVideoName}
                            </strong>
                        </div>
                        <div className="sidebar__summary-item">
                        ...
                        <div className="sidebar__summary-item">
                            ...
                            <strong className="sidebar__summary-value">{canvas.width}×{canvas.height}</strong>
                        </div>
                    </div>
                </div>
            )}
            ...
            {zones.length > 0 && (
                <div className="sidebar__section">
                    <div className="sidebar__section-title">{isClipMode ? 'Clip Layers' : `Layers (${zones.length})`}</div>
                    <div className="sidebar__zone-list">
                        {[...zones].sort((a, b) => b.z - a.z).map((zone) => (
                            <div
                                key={zone.id}
            ...
            {!isClipMode && cloudEnabled && (
                <div className="sidebar__section">
                    <div className="sidebar__section-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {cloudError && (
                        <div className="sidebar__cloud-error">{cloudError}</div>
                    )}
                            ...
                            >
                                ...
                                {deleteConfirm === t.docId ? (
                                    <div className="sidebar__cloud-item-confirm">
                                        <button onClick={() => handleDeleteFromCloud(t.docId)} className="sidebar__cloud-delete-yes">Yes</button>
                                        <button onClick={() => setDeleteConfirm(null)} className="sidebar__cloud-delete-no">No</button>
                                    </div>
                                ) : (
                                    <button
            ...
            <div className="sidebar__section" style={{ marginTop: 'auto' }}>
                <div className="sidebar__section-title">{isClipMode ? 'Edit Actions' : 'Actions'}</div>
                <div className="sidebar__actions">
                    {!isClipMode && cloudEnabled && (
                        <button className="sidebar__action-btn sidebar__action-btn--primary" onClick={handleSaveToCloud} disabled={saving}>
                            {saving ? <Loader2 size={15} className="sidebar__spinner" /> : <CloudUpload size={15} />}
                    ...
                    {!isClipMode && previewEnabled && (
                        <button className="sidebar__action-btn sidebar__action-btn--accent" onClick={onTest}>
            ...
            {!isClipMode && manifestOpen && <ManifestLoader onClose={() => setManifestOpen(false)} />}
