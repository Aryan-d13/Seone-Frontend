...
import { X, Maximize, Layers, Clock, AlertTriangle } from 'lucide-react';
...
export default function PreviewPanel({ onClose }: Props) {
  ...
  const [status, setStatus] = useState<
    'idle' | 'validating' | 'rendering' | 'done' | 'error'
  >('idle');
        ...
        <div className="preview-panel__body">
          ...
          {status === 'error' && (
            <div className="preview-panel__error">
              ...
              {errorMsg && <div className="preview-panel__error-details">{errorMsg}</div>}
            </div>
          )}
          ...
          {status === 'idle' && (
            <div
              className="preview-panel__loading"
              style={{ color: 'var(--text-secondary)' }}
            >
              <span style={{ fontSize: 14 }}>
                Configure settings below, then click Validate or Render
              </span>
            </div>
          )}
