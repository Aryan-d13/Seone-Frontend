...
export function exportTemplate(template: TemplateJSON): string {
  const output = {
    ...
    ...(template.slot_contract
      ? { slot_contract: JSON.parse(JSON.stringify(template.slot_contract)) }
      : {}),
    ...(template.compatibility_key
      ? { compatibility_key: template.compatibility_key }
      : {}),
  };
