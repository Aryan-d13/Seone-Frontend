...
function validateTextSpec(text: unknown): TextSpec {
  ...
  return {
    ...
    horizontal_align: (['left', 'center', 'right'].includes(String(t.horizontal_align))
      ? String(t.horizontal_align)
      : 'center') as TextSpec['horizontal_align'],
    vertical_align: (['top', 'middle', 'bottom'].includes(String(t.vertical_align))
      ? String(t.vertical_align)
      : 'middle') as TextSpec['vertical_align'],
...
function validateMediaSpec(media: unknown): MediaSpec {
  ...
  return {
    ...
    crop_anchor: (['center', 'top', 'bottom'].includes(String(m.crop_anchor))
      ? String(m.crop_anchor)
      : 'center') as MediaSpec['crop_anchor'],
    ...(cropFocus
      ? {
          crop_focus: {
            x:
...
function validateZone(raw: unknown, index: number): ZoneSpec {
  ...
  const zone: ZoneSpec = {
    ...
    bounds: {
      ...
      ...(!forceAutoHeight && bounds.height != null
...
export function importTemplate(jsonString: string): TemplateJSON {
  ...
  for (const [key, value] of Object.entries(rawAssets)) {
    assets[key] = {
      ...
      path: value.path || '',
      ...(readOptionalString(value.source_uri)
        ? { source_uri: readOptionalString(value.source_uri) }
        : {}),
      ...(readOptionalString(value.gcs_path)
        ? { gcs_path: readOptionalString(value.gcs_path) }
        : {}),
      ...(readOptionalString(value.family)
        ? { family: readOptionalString(value.family) }
        : {}),
      ...(value.weight != null && Number.isFinite(Number(value.weight))
        ? { weight: Number(value.weight) }
        : {}),
      ...(readOptionalString(value.style)
  ...
  return {
    ...
    styles,
    assets,
    ...(data.slot_contract && typeof data.slot_contract === 'object'
      ? { slot_contract: data.slot_contract as TemplateJSON['slot_contract'] }
      : {}),
    ...(typeof data.compatibility_key === 'string' && data.compatibility_key.trim()
      ? { compatibility_key: data.compatibility_key.trim() }
      : {}),
  };
}
