...
describe('protectedAssetLoader', () => {
  ...
  it('loads and caches protected asset blobs via authFetch', async () => {
    ...
    const first = await acquireProtectedAssetUrl(
      'http://localhost:8000/api/v1/pages/admin/templates/chaturnath_v1/assets/logo_mark'
    );
    const second = await acquireProtectedAssetUrl(
      'http://localhost:8000/api/v1/pages/admin/templates/chaturnath_v1/assets/logo_mark'
    );
  ...
  it('classifies auth failures as unauthorized asset errors', async () => {
    authFetchMock.mockRejectedValue(
      new Error(
        'API returned 401 for /api/v1/pages/admin/templates/chaturnath_v1/assets/logo_mark. Auth suspicion signaled.'
      )
    );
